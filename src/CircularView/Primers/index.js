...
function Primers({radius, 
    ...
    Object.keys(primers).forEach(function(key, index) {
        ...
        if (spansOrigin) {
            primerITree.add(startAngle, expandedEndAngle, undefined, {...annotationCopy})
        } else {
            //normal primer
            // we need to add it twice to the interval tree to accomodate primers which span the origin
            primerITree.add(startAngle, expandedEndAngle, undefined, {...annotationCopy})
            primerITree.add(startAngle + 2 * Math.PI, expandedEndAngle + 2 * Math.PI, undefined, {...annotationCopy})
        }
    ...
    return {
        component: <g className='vePrimers' key='vePrimers'>
            {svgGroup}
        </g>,
